...
def enrichment_search():
    ...
    hq = pd.read_table('data/high_quality_candidates.txt.xz', header=None)
    q = pd.read_table('data/quality_candidates.txt.xz', header=None)
    qc = pd.concat([hq, q])
...
def plot_qual_homologs():
    ...
    qualassess = pd.read_table('data/quality_assessment.tsv.xz')
