def load_data():
    import pandas as pd
    lI = pd.read_table('output_clustering_significance_levelI.tsv')
    lII = pd.read_table('output_clustering_significance_levelII.tsv')
    lIII = pd.read_table('output_clustering_significance_levelIII.tsv')
    return lI, lII, lIII
...
def siggraph(df):
    ...
    plt.tight_layout()
    plt.savefig('significance_across_clustering_levels.svg')
    plt.close()
...
def idgraph(lI, lII, lIII):
    ...
    plt.xlabel('Identity')
    plt.tight_layout()
    plt.savefig('identity_distribution.svg')
    plt.close()    
...
def covgraph(lI, lII, lIII):
    ...
    plt.xlabel('Coverage')
    plt.tight_layout()
    plt.savefig('coverage_distribution.svg')
    plt.close()
