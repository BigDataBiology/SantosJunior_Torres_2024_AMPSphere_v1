...
def terminal_place_process(ref):
    ...
    for chunk in pd.read_table('data/result.tsv.gz',
                               sep='\t',
                               header=None,
                               names=['gmsc', 'status'],
...
def RNAcode(ref):
    '''
    Processes the results from the 
    RNACode quality test of AMPs
    also listing those not tested
    '''
    import pandas as pd

    red_amps = set()
    for chunk in pd.read_table('data/rnacode_seq_false.tsv.xz',
                               header=None,
                               chunksize=10_000_000):
        chunk = chunk[chunk[0].isin(ref.gmsc)][0]
        amps = set(ref[ref.gmsc.isin(chunk)].amp)
        red_amps = red_amps.union(amps)

    green_amps = set()
    for chunk in pd.read_table('data/rnacode_seq.tsv.xz',
                               header=None,
                               chunksize=10_000_000):
        chunk = chunk[chunk[0].isin(ref.gmsc)][0]
        amps = set(ref[ref.gmsc.isin(chunk)].amp)
    ...
    all_amps = set(ref.amp)
    annotated = green_amps.union(red_amps)
    yellow_amps = all_amps - annotated

    testresult = dict()
    for i in red_amps: testresult[i] = 'Failed'
    for i in green_amps: testresult[i] = 'Passed'
    for i in yellow_amps: testresult[i] = 'Not tested'    

    df = pd.DataFrame.from_dict(testresult,
                                orient='index')
    df = df.reset_index()
    df = df.rename({'index': 'AMP', 0: 'RNAcode'}, axis=1)

    df.to_csv('rnacode_list.tsv',
              sep='\t',
              header=True,
              index=None)
